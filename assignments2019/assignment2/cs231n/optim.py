...
def sgd_momentum(w, dw, config=None):
    ...
    v = config['momentum'] * v - config['learning_rate'] * dw # integrate velocit
    next_w = w + v
...
def rmsprop(w, dw, config=None):
    ...
    grad_squared = decay_rate * config['cache'] + (1 - decay_rate) * dw * dw
...
#     beta1 = config['beta1']
#     beta2 = config['beta2']
#     first_moment = beta1 * config['m'] + (1 - beta1) * dw
#     second_moment = beta2 * config['v'] + (1 - beta2) * dw * dw
#     first_unbias = first_moment / (1 - beta1 ** config['t'])
#     second_unbias = second_moment / (1 - beta2 ** config['t'])
