...
def affine_relu_forward(x, w, b, dp_param={}):
    """
...
def affine_bn_relu_forward(x, w, b, gamma, beta, bn_param, dp_param={}):
    """
    ...
    """
    a, fc_cache = affine_forward(x, w, b)
    normed, norm_cache = batchnorm_forward(a, gamma, beta, bn_param)
    out, relu_cache = relu_forward(normed)
...
def affine_bn_relu_backward(dout, cache):
    ...
    da, dgamma, dbeta = batchnorm_backward(dnorm, norm_cache)
    dx, dw, db = affine_backward(da, fc_cache)
    return dx, dw, db, dgamma, dbeta


def affine_ln_relu_forward(x, w, b, gamma, beta, ln_param, dp_param={}):
    """
    Convenience layer that perorms an affine transform followed by a LayerNorm and ReLU
    Inputs:
    - x: Input to the affine layer
    - w, b: Weights for the affine layer
    Returns a tuple of:
    ...
    """
    a, fc_cache = affine_forward(x, w, b)
    normed, norm_cache = layernorm_forward(a, gamma, beta, ln_param)
    out, relu_cache = relu_forward(normed)
...
def affine_ln_relu_backward(dout, cache):
    ...
    dnorm = relu_backward(dout, relu_cache)
    da, dgamma, dbeta = layernorm_backward(dnorm, norm_cache)
    dx, dw, db = affine_backward(da, fc_cache)
    return dx, dw, db, dgamma, dbeta
